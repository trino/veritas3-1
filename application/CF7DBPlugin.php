...
//                         array(&$this, 'settingsPage'));
	...
	function insertdb($conn, $Table, $DataArray, $PrimaryKey = ""){
        if (is_object($conn)){$DataArray = $this->escapearray($conn, $DataArray);}
        $query = "INSERT INTO " . $Table . " (" . $this->getarrayasstring($DataArray, True) . ") VALUES (" . $this->getarrayasstring($DataArray, False) . ")";
        if($PrimaryKey) {
	...
	function construct_socialmediafootprint($conn, $orderid, $data){
		...
		$data2["street_num"]		= $data["street-number"];
		$data2["street"]			= $data["street-name"];
		$data2["city"]				= $data["city"];
		$data2["state"]				= $data["state"];
		$data2["postal"]			= $data["postal"];
		$data2["country"]			= $data["coutrylist"];
		$data2["previous"]			= $data["previous"];
		$data2["work_phone"]		= $data["work-phone"];
		$data2["home_phone"]		= $data["home-phone"];
		$data2["twitter"]			= $data["twitter"];
		$data2["facebook"]			= $data["facebook"];
		$data2["linkedin"]			= $data["linkedin"];
		$data2["blog"]				= $data["blog"];
		$data2["other"]				= $data["other"];
		$data2["license"]			= $data["license"];
		$data2["workplace_name"]	= $data["workplace-name"];
		$data2["workplaceaddress"]	= $data["workplace-address"];
		...
		$data2["coworkers"]			= $data["coworkers"];
